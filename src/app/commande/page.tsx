        ...
        <aside className="h-fit rounded-2xl border border-gray-100 bg-white p-4 shadow-sm sm:rounded-[2rem] sm:p-6 lg:sticky lg:top-44">
          ...
          <div className="mt-4 space-y-3 sm:mt-6 sm:space-y-4">
            {cart.map((item, index) => (
              <div
                key={`${item.productId}-${item.selectedSize}-${index}`}
                className="grid grid-cols-[66px_1fr] gap-3 border-b border-gray-100 pb-3 sm:grid-cols-[76px_1fr] sm:gap-4 sm:pb-4"
              >
                <div className="overflow-hidden rounded-xl bg-[#f4efe7] sm:rounded-2xl">
                  {item.productImage ? (
                    <img
